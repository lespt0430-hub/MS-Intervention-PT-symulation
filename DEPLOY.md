...
> 없습니다. 학생 이름·점수도 **기본 설정에서는** 그 PC 브라우저에만 남습니다.
>
> 다만 **결과 자동 수집(`PROFESSOR_SETUP.md`)을 켜면** 학생 이름·점수가
> 교수님 구글 시트로 전송됩니다. 시트는 교수님 계정 안에만 있고 조회에
> 비밀번호가 필요하지만, 공개 저장소의 `config.js`에는 수집 주소가 적힙니다.
> 켜기 전에 학생들에게 수집 항목과 용도를 안내해 주세요.
>
> 그리고 **CPG 원문을 그대로 옮겨 적은 부분이 있다면** 저작권을 한 번
