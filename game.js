...
function buildPatientFigure(patient, opts) {
  ...
  // ── 하의 ──
  // 상의 밑단과 허벅지 사이를 덮는다. 이게 없으면 치료사는 스크럽 상의
  // 밑단(z=0.92)과 몸통 끝(z=1.00) 사이로 피부색 몸통이 8cm 드러나
  // 바지를 안 입은 것처럼 보인다. (환자복은 1.06까지 내려와 가려졌을 뿐,
  // 원인은 같다 — 그래서 둘 다 입힌다.)
  //
  // 허리는 상의 안쪽에 들어가야 하므로 상의(반지름 0.24)보다 좁게 잡는다.
  lathe([
    [0.205, 0.720],                      // 허리 — 상의 속에 들어간다
    [0.212, 0.820],
    [0.214, 0.900],                      // 엉덩이
    [0.200, 0.970],
    [0.165, 1.020],                      // 허벅지가 시작되는 곳
  ], M.pants, 0.52, 0.10);
