...
const SHOTS = [
  ['입구',     0.0, -8.5, 180, -3],
  ['전기치료실', 0.0, -1.0, 180, -5],
  ['운동치료실', 8.3, -8.4, 180, -5],   // 실 전체가 한눈에 들어오게 뒤로 물림
  ['운동기구',   9.5, -2.0, 180, -5],   // 짐볼(p16) 쪽
  ['트레드밀',   9.10, -9.20,  77, -3],   // p10 — 트레드밀 벨트 위에 발이 얹혔는지
  ['리포머',    10.60, -5.45, 270, -4],  // p12 — 거울 벽 쪽 새 자리 (옆에서)
  ['수치료실',   5.4, 3.7,  200, -6],
  ['수치료입구', 5.4, 1.2,  180, -2],
  ['수치료들어서', 8.2, 4.3, 248, -4],   // 문을 막 지났을 때 표민아가 보이나
  ['벽너머',     8.6, 4.2, 200, -1],     // 수치료실에서 운동치료실 벽 쪽을 봄
  ['케이블',     6.6, -0.3,  90, -3],
  ['보행풀',     5.9, 3.8,  180, -5],   // 관찰창 정면 — 환자 머리가 보이는지 확인
  ['전신풀',     8.2, 4.6,  200,  -8],   // 물속 치료사와 환자가 같이 보이게
  ['도수복도',  -5.70, -4.6, 180, -5],
  ['도수룸',   -9.4, -3.6,  90, -5],
  ['도수현수',  -9.6, -3.6,  90,  9],   // 베드 위 천장 현수장치가 걷어 올려져 있는지
  ['도수현수옆', -13.8, -3.6, 270,  8], // 창가(머리쪽)에서 본 레일 두 줄의 길이
  ['특수치료',  -1.2, 2.2,  160, -6],
  ['충격파',    1.1, 7.6,  -47, -8],
  ['기구라인',   8.5, -5.6,   0, -3],
  // 환자가 19명으로 늘며 새로 생긴 자리들
  ['도수5실',   -9.4, 7.2,   90, -5],   // p13 경인성 두통 — 늘어난 다섯 번째 방
  ['경추견인',  -1.3, 0.8,  110, -6],   // p14 — 머리맡 경추 견인기
  ['요추견인',  -1.3, 6.4,  110, -6],   // p15 — 머리맡 요추 견인기
  ['균형훈련',  11.3, -7.9,   0, -8],   // p18 — 평행봉 사이 워블보드
  ['짐볼',       9.3,  0.5, 180, -6],   // p16 — 파란 매트 위 짐볼 체간 협응 운동
  ['늑목',      11.2,  0.9, 180, -4],
  ['명패뒤',   -12.4, -3.60, 270, -9],   // 명패 뒷면 — 반대쪽에서도 이름이 읽히는지
  ['실간판',     0.0,  4.80, 180, 12],   // 전기치료실 간판 — 실 색 강조선과 영문 부제   // 명패 뒷면 — 반대쪽에서도 이름이 읽히는지   // p20 — 늑목 앞 측만증 3차원 자가교정
  ['밴드',      12.20, -3.40, 205, -2],   // p21 — 거울 오른쪽 벽 탄성밴드 스테이션
  ['케이블어깨', 5.6,  1.6, 180, -3],   // p22 — 케이블 타워 앞 돌림근띠 강화
  ['접수데스크', 1.3, -9.5, 275, -4],   // 앉아서 전산 처리하는 접수 직원 (말 걸기 범위 안)
  ['안내판',    -1.9, -9.6, 100, -2],   // 정수기 옆으로 옮긴 진료 안내판
];
